...
function inferForProject () {
    ...
    # 清除上次编译数据
    if [ -d ./infer-out ]; then
        echo -e $COLOR_SUCC'-----清除上次编译数据infer-out-----'$COLOR_SUCC
        rm -rf ./infer-out
    fi
    ...
    if [ -f ./infer-out/report.html/index.html ]; then
    ...
    open -a "/Applications/Safari.app" ./infer-out/report.html/index.html
}
